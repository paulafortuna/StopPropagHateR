...
# Create two empy lists to store the filenames and the models/tokenizers
modelsList <- list()
tokenizerList <- list()


# Load the model referenced by the filename if it's not loaded, else return the already loaded model
loadOrReuseKerasModel <- function(model_filename) {
  if (is.null(modelsList[[model_filename]])) {
    model <- kerasR::keras_load(model_filename)
    modelsList[[model_filename]] <<- model
    return(modelsList[[model_filename]])
  } else {
    return(modelsList[[model_filename]])
  }
}
...
# Load the tokenizer referenced by the filename if it's not loaded, else return the already loaded tokenizer
loadOrReuseKerasTokenizer <- function(tokenizer_filename) {
  if (is.null(tokenizerList[[tokenizer_filename]])) {
    tokenizer <- keras::load_text_tokenizer(tokenizer_filename)
    tokenizerList[[tokenizer_filename]] <<- tokenizer
    return(tokenizerList[[tokenizer_filename]])
  } else {
    return(tokenizerList[[tokenizer_filename]])
  }
}
            ...
            if(length(intersect(language, c("en", "pt", "racism"))) == 0){
              ...
              try(
                if(!dir.exists(folder_name)){
                  ...
                  tokenizer <- loadOrReuseKerasTokenizer(tokenizer_filename)
                  ...
                  mod <- loadOrReuseKerasModel(model_filename)
...
stopPropagHateMessage <- function(message, hate_type = "hate", language = "en"){
  ...
  return(stopPropagHateWithoutPrompt(texts, hate_type,  language)[1,1])
}
